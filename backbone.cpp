...
//   int H = img.rows;
//   int W = img.cols;
//   int C = img.channels();  // 通常为 3

//   // 2. 转 float32 并归一化 [0,1]
//   cv::Mat img_f32;
//   //   img.convertTo(img_f32, CV_32F, 1.0f / 255.0f);
//   img.convertTo(img_f32, CV_32F);

//   // 3. 转 float16（CV_16F）
//   cv::Mat img_f16;
//   img_f32.convertTo(img_f16, CV_16F);

//   // 4. 计算 NCHW 大小
//   size_t elemCount = static_cast<size_t>(C) * H * W;  // C*H*W
//   size_t dataSize = elemCount * sizeof(uint16_t);  // float16 每元素 2 字节
//   if (dataSize != inputBufferSize_b) {
//     std::cerr << "Error: 预处理后数据大小(" << dataSize << ")与设备缓冲区大小("
//               << inputBufferSize_b << ")不匹配\n";
//     return FAILED;
//   }

//   // 5. 分配临时 Host/Device 内存缓冲
//   void* hostBuffer = nullptr;
//   aclError aclRet;
//   aclRet = aclrtMallocHost(&hostBuffer, dataSize);
//   // 6. 手动做 HWC->NCHW 排序
//   auto srcPtr = reinterpret_cast<uint16_t*>(img_f16.data);
//   auto dstPtr = reinterpret_cast<uint16_t*>(hostBuffer);
//   // 原始内存布局: for h in [0,H) for w in [0,W) for c in [0,C): src[h*W*C + w*C
//   // + c] 目标布局: for c in [0,C) for h in [0,H) for w in [0,W): dst[c*H*W +
//   // h*W + w]
//   for (int c = 0; c < C; ++c) {
//     size_t offsetC = static_cast<size_t>(c) * H * W;
//     for (int h = 0; h < H; ++h) {
//       size_t offsetH = static_cast<size_t>(h) * W;
//       for (int w = 0; w < W; ++w) {
//         size_t dstIdx = offsetC + offsetH + w;
//         size_t srcIdx = (static_cast<size_t>(h) * W + w) * C + c;
//         dstPtr[dstIdx] = srcPtr[srcIdx];
//       }
//     }
//   }

//   aclRet = aclrtMemcpy(inputBuffer_b, dataSize, hostBuffer, dataSize,
//                          ACL_MEMCPY_HOST_TO_DEVICE);
//   aclrtFreeHost(hostBuffer);
//   if (aclRet != ACL_SUCCESS) {
//     std::cerr << "Error: 数据拷贝失败，code=" << static_cast<int>(aclRet)
//               << std::endl;
//     return FAILED;
//   }
...
Result Backbone::backbone_Inference()
{
    ...
    std::cout<<"+++++++++++++++++++inputBufferSize_b: "<<inputBufferSize_b<<std::endl;
    std::cout<<"+++++++++++++++++++imageBytes size: "<<sizeof(imageBytes)<<std::endl;
...
float* Backbone::backbone_GetResults()
{
    aclError ret;
...
float* Backbone::runBackbone(cv::Mat& img)
{

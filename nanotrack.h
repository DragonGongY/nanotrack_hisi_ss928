...
class NanoTrack {
   public:
    NanoTrack(const char* Tback_model, const char* Xback_model, const char* Head_model);
    ~NanoTrack();
    void initsource();
   ...
   private:
    ...
    cv::Mat get_subwindow(const cv::Mat& im, cv::Point2f pos, int model_sz, int original_sz,
                          cv::Scalar avg_chans);
    std::vector<float> convert_score(const cv::Mat& score);
    cv::Mat convert_bbox(const cv::Mat& delta, const cv::Mat& point);
    std::tuple<float, float, float, float> bbox_clip(float cx, float cy, float width, float height,
                                                     cv::Size boundary);
};
